...
A Rust CLI (`autoqa`) that drives a real Chrome browser through `claude -p` +
...
`autoqa run` spawns `claude -p` wired to Playwright MCP (`npx @playwright/mcp@latest`,
...
- `--isolated` is load-bearing: without it, Playwright MCP persists the
  browser profile to disk and reuses it across separate `autoqa run`
